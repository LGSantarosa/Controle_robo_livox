...
namespace hoverboard_driver
{
  ...
  void hoverboard_driver::protocol_recv(const rclcpp::Time &time, char byte)
  {
    ...
    if (msg_len == sizeof(SerialFeedback))
    {
      ...
      if (msg.start == START_FRAME && msg.checksum == checksum)
      {
        ...
        // O firmware padrao NAO manda contagem de encoder, so velocidade.
        // Integra a velocidade p/ ter uma posicao FINITA (senao o controlador
        // recebe NaN e rejeita os comandos). A odometria real vem do FAST-LIO.
        static double last_frame_sec = -1.0;
        double now_sec = time.seconds();
        if (std::isnan(hw_positions_[left_wheel]))  hw_positions_[left_wheel] = 0.0;
        if (std::isnan(hw_positions_[right_wheel])) hw_positions_[right_wheel] = 0.0;
        if (last_frame_sec > 0.0)
        {
          double dt = now_sec - last_frame_sec;
          if (dt > 0.0 && dt < 1.0)
          {
            hw_positions_[left_wheel]  += hw_velocities_[left_wheel]  * dt;
            hw_positions_[right_wheel] += hw_velocities_[right_wheel] * dt;
          }
        }
      ...
      const rclcpp::Time &time, const rclcpp::Duration &period)
  {
    ...
    // Compensacao de deadband PRESERVANDO A CURVA. O motor so vence a inercia
    // acima de ~speed 60-80 (medido no fio). Em vez de elevar cada roda separada
