...
| 3 | ✅ **FEITA (18-09)** — URDF completo girado (§3, decisão 051) + geometria canônica `robot_base/config/geometria_robo3.yaml` (footprint sem padding, decisão 052) + testes reescritos | modelo e marcha concordam; o contorno existe como artefato legível por máquina, travado contra o URDF (cobre corpo e varredura, não carrega folga). 🔄 **Fronteira mudada em 18-09:** o Nav2 **consumir** o contorno e o `robot_state_publisher` saíram daqui para a etapa 4, onde nascem o perfil e o bringup que os consomem | não |
| 4 | Perfis `robo2`/`robo3` e **um bringup único** do robô 3 — plano próprio aprovado em 18-09: `docs/PLANO_ETAPA4_ROBO3.md` | RSP + MEGA + `cmd_vel_to_wheels` + Xbox/direcional + mux único num lugar só. **Herdado da etapa 3:** o perfil Nav2 do robô 3 **lê** `geometria_robo3.yaml` ou é amarrado a ele por teste de igualdade (vértices nunca redigitados), e declara `footprint_padding` **explícito** — ausente, o Nav2 aplica o padrão dele como margem escondida. Idem `collision_monitor`, meia largura, corredor de ré e recuo do para-choque | não |
...
| 6 | Ensinar a `pilha` a escolher `sim_robo3` (`robo:=3`, `use_sim_time`, qual atuador encerra) | a etapa 3 da v1, agora possível. **Herdado da etapa 4** (`PLANO_ETAPA4_ROBO3.md` §3): `ros2 param get` do `footprint` e do `footprint_padding` **vivos** nos dois costmaps do robô 3 — na 4 só o YAML reescrito é provado | não |
