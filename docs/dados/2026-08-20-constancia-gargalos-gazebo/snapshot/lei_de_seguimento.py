...
def alvo_estavel_de_passagem(caminho, passagem, x, y, saida=0.60,
    ...
    fixo ``saida`` metros depois do vão. Antes de adotá-lo, projeta a reta até
    o plano central da passagem. Se ela ainda não cabe com corpo+margem, mira
    primeiro o centro do próprio plano; assim uma aproximação lateral nunca é
    transformada em atalho pela quina.

    Devolve ``(alvo, fase)`` onde fase é ``'centro'`` ou ``'eixo'``. Não há
    estado temporal aqui: para a mesma pose e passagem a decisão é idêntica.
    Quem trava qual passagem está ativa é a cola ROS.
    """
    if not caminho:
    ...
    # estreito já ficou para trás e manter o eixo é sempre o lado estável.
    if s < 0.0:
