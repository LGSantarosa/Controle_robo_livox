...
**Status**: aplicada em código, **NÃO testada**. Default continua `frente:=1.0`,
ou seja: quem não passar o argumento não sente diferença nenhuma.
...
- **Vale só para o robô 3**, que é teleop puro: sem Livox, sem `pose_estimator`,
  sem `/odom`. No **robô 2** o mesmo argumento seria insuficiente — a frente
  física passaria a ser a traseira e o Livox ficaria olhando para trás; lá
  o giro de 180° tem de ir junto no TF `base_link`→lidar e no URDF.
- Nada foi ao robô: o teste é `bash bin/sobe-robo3 frente:=-1.0` e LB +
